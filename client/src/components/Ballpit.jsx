...
class x {
  ...
  #p() {
    if (this.isDisposed) return; // ne rien faire si déjà démonté
    if (this.#e.canvas) {
  ...
  resize() {
    ...
    try {
      ...
      console.error("Three: Failed to set renderer size:", error);
    }
  ...
  dispose() {
    this.isDisposed = true; // avant tout, pour couper le retry en vol
    this.#y();
    this.#z();
    this.#c.dispose();
    this.clear();
...
const Ballpit = ({ className = "", followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // Attend une frame pour laisser le double-montage StrictMode et la
    // mise en page se stabiliser avant de créer le contexte WebGL.
    const raf = requestAnimationFrame(() => {
      if (cancelled || !canvasRef.current) return;
      instance = createBallpit(canvasRef.current, {
        followCursor,
        ...props,
      });
